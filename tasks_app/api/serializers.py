    ...
    def validate(self, data):
        ...
        if assignee_id:
            ...
            if not board.members.filter(id=assignee_id).exists():
                raise serializers.ValidationError({'assignee_id': 'User must be a member of the board.'})
        ...
        if reviewer_id:
            ...
            if not board.members.filter(id=reviewer_id).exists():
                raise serializers.ValidationError({'reviewer_id': 'User must be a member of the board.'})
